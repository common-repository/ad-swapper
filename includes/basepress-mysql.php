...
// *****************************************************************************
    ...
    namespace greatKiwi_byFernTec_adSwapper_local_v0x1x211_basepressMysql ;
...
EOT;
    ...
    //          each of the value in $data. If string, that format will be used
    //          for all of the values in $data. If omitted, all values in $data
    //          will be treated as strings unless otherwise specified in
    //          wpdb::$field_types.
    //
...
//
//  Removed 20 July 2014 because it generates:-
//      Notice: wpdb::escape is deprecated since version 3.6! Use
//      wpdb::prepare() or esc_sql() instead. in
//      /opt/lampp/htdocs/plugdev/wp-includes/functions.php on line 3078

    // -------------------------------------------------------------------------
    // From:-
    //      http://dev.mysql.com/doc/refman/5.0/en/identifiers.html
    //
    // it appears that table names may contain:-
    //
    //      ASCII: [0-9,a-z,A-Z$_]
    //      (basic Latin letters, digits 0-9, dollar, underscore)
    //
    // Plus various other stuff too.  But to keep things simple, we'll
    // restrict them to alphanumeric and underscore chars only.
    // -------------------------------------------------------------------------
    ...
    if ( ! \greatKiwi_byFernTec_adSwapper_local_v0x1x211_stringUtils\ctype_alphanumeric_underscore(
                $table_name
                )
        ) {

        $msg = <<<EOT
Bad table name (contains invalid characters)
EOT;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $table_name , '$table_name' ) ;

//global $wpdb ;
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $wpdb->prefix , '$wpdb->prefix' ) ;
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $wpdb->base_prefix , '$wpdb->base_prefix' ) ;
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $wpdb->blogid , '$wpdb->blogid' ) ;
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $wpdb->siteid , '$wpdb->siteid' ) ;
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( '"' . is_multisite() . '"' , 'is_multisite()' ) ;
...
function delete_records(
    ...
    //
    //      $where
    //          An ARRAY of field NAME=VALUE pairs.  The resulting database
...
function get_record_count(
    ...
    //      On SUCCESS
    //          STRING $record_count
    //              NOTE! The count is returned as a string - so that we (= PHP)
    //              can cope with large datasets ($count > PHP_INT_MAX).
    //
    //      On FAILURE
    //          ARRAY( $error_messsage STRING )
    // -------------------------------------------------------------------------
...
EOT;
    ...
    if ( $count === NULL ) {
        ...
        return <<<EOT
PROBLEM:&nbsp; MySQL failure getting record count (perhaps the specified table doesn't exist)
For MySQL table:&nbsp; {$safe_table_name}
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
