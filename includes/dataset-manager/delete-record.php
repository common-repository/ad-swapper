...
function delete_record(
    ...
    // -------------------------------------------------------------------------
    // \greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\delete_record(
    //      $caller_app_slash_plugins_global_namespace      ,
    //      $home_page_title                                ,
    //      $caller_apps_includes_dir                       ,
    //      $all_application_dataset_definitions            ,
...
//pr( $all_application_dataset_definitions ) ;
    ...
    // =========================================================================
    // Get the (top level) dataset's "STORAGE METHOD"...
    // =========================================================================

    if (    array_key_exists( 'storage_method' , $top_level_datasets_dmdd )
            &&
            $top_level_datasets_dmdd['storage_method'] === 'mysql'
        ) {
        $top_level_storage_method = 'mysql' ;

    } else {
        $top_level_storage_method = 'array-storage' ;
...
EOT;
    ...
    if ( $_GET[ $key_field_get_var_name ] !== '*a*l*l*' ) {
        ...
        if ( $top_level_storage_method === 'mysql' ) {

            // -----------------------------------------------------------------

            require_once( $caller_apps_includes_dir . '/dataset-manager/mysql-array-storage-key-field-overrides-support.php' ) ;
...
*/
            ...
            $record_key_ok =
                \greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\is_record_key(
                    $_GET[ $key_field_get_var_name ]
                    ) ;
        ...
        if ( $record_key_ok !== TRUE ) {

            $msg = <<<EOT
PROBLEM Deleting Dataset Record:&nbsp; Bad "record_key"
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
...
//      $top_level_dataset_title         = $top_level_dataset_title ;
        $top_level_dataset_records       = array() ;
        $top_level_key_field_slug        = '' ;
        $top_level_record_indices_by_key = array() ;
    ...
    if ( $_GET[ $key_field_get_var_name ] === '*a*l*l*' ) {
        ...
        if (    $top_level_storage_method !== 'mysql'
                &&
                ! array_key_exists( $_GET[ $key_field_get_var_name ] , $top_level_record_indices_by_key )
            ) {
...
EOT;
    ...
    if ( $top_level_storage_method === 'mysql' ) {
        ...
        foreach ( $record_keys_to_delete__by_dataset_slug as $this_dataset_slug => $record_keys_to_delete ) {
            ...
            require_once( $caller_apps_includes_dir  . '/basepress-mysql.php' ) ;
            ...
            if ( ! array_key_exists( $this_dataset_slug , $all_application_dataset_definitions ) ) {

                $msg = <<<EOT
PROBLEM:&nbsp; Unrecognised/unsupported "dataset_slug"
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
            ...
            $selected_datasets_dmdd =
                $all_application_dataset_definitions[ $this_dataset_slug ]
                ;
            ...
            // =================================================================
            // Get the dataset's MySQL "key" field...
            // =================================================================
...
/*
            ...
            if (    array_key_exists( 'mysql_overrides' , $selected_datasets_dmdd )
                ...
                $msg = <<<EOT
PROBLEM:&nbsp; Can't find "key" field of record(s) to delete
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
...
EOT;
            ...
            $sql = <<<EOT
DELETE FROM `{$this_mysql_table_name}`
WHERE `{$mysql_key_field_slug}` IN ({$in})
EOT;
            ...
            $number_records_affected = $wpdb->query( $sql ) ;
                                            ...
                                            //  rows affected/selected.  If
                                            //  there is a MySQL error, the
            ...
            if ( $number_records_affected === FALSE ) {
                ...
                $msg = <<<EOT
PROBLEM:&nbsp; Delete records failure (#1)
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\pr( $number_records_affected ) ;
//echo '<br />' , gettype( $number_records_affected ) ;

            // -----------------------------------------------------------------

            if ( $number_records_affected !== count( $record_keys_to_delete ) ) {

                $msg = <<<EOT
PROBLEM:&nbsp; Delete records failure (#2)
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\pr( $record_keys_to_delete__by_dataset_slug ) ;
    ...
    if ( $parent_dataset_storage_method === 'mysql' ) {

        return <<<EOT
PROBLEM:&nbsp; Deleting child records for MySQL stored datasets not implement yet!
For dataset:&nbsp; {$parent_dataset_title}
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;

        //  TODO !!!
        //
        //  Make MySQL records with children deletable
