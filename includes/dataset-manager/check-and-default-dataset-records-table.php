...
function check_and_default_dataset_records_table(
    ...
    if ( array_key_exists( 'dataset_records_tables_by_role' , $selected_datasets_dmdd ) ) {
        ...
        if ( ! is_array( $selected_datasets_dmdd['dataset_records_tables_by_role'] ) ) {
            ...
            return <<<EOT
PROBLEM: Bad "dataset_records_tables_by_role" (not an array)
...
EOT;
        ...
        if ( count( $selected_datasets_dmdd['dataset_records_tables_by_role'] ) < 1 ) {

            return <<<EOT
PROBLEM: Empty "dataset_records_tables_by_role"
For dataset:&nbsp; {$dataset_title}
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
        ...
        if ( ! \function_exists( '\\is_user_logged_in' ) ) {
            ...
            return <<<EOT
PROBLEM: WordPress not running ("dataset_records_tables_by_role" requires WordPress)
For dataset:&nbsp; {$dataset_title}
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
        ...
        if ( ! \is_user_logged_in() ) {
            ...
            return <<<EOT
PROBLEM: You must be LOGGED-IN to view this page...
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
        ...
        \get_currentuserinfo() ;
            // Retrieves the information pertaining to the currently logged in
            // user, and places it in the global variable $current_user.
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $dataset_title ) ;
