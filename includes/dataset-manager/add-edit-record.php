...
// =============================================================================
    ...
    require_once( dirname( __FILE__ ) . '/add-edit-record_get-record-to-be-edited.php' ) ;
...
function add_edit_record(
    ...
    // -------------------------------------------------------------------------
    // \greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\add_edit_record(
    //      $caller_app_slash_plugins_global_namespace      ,
    //      $home_page_title                                ,
    //      $caller_apps_includes_dir                       ,
    //      $all_application_dataset_definitions            ,
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $display_options ) ;
...
//pr( $all_application_dataset_definitions ) ;
    ...
    $dataset_manager_home_page_title = $error_page_title ;
...
//pr( $dataset_records ) ;
    ...
    // -------------------------------------------------------------------------
    // \greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\get_dataset_key_field_slug(
    //      $all_application_dataset_definitions    ,
    //      $dataset_slug
    //      )
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    // =========================================================================
    // GET the DATASET's RECORD INDICES BY KEY (if needed)...
    // =========================================================================
...
//pr( $_POST ) ;
        ...
        foreach ( $array_storage_field_indices_to_base64_encode_pre_check as $field_index ) {
            ...
            $question_field_base64_encoded = FALSE ;

            // -----------------------------------------------------------------

            if (    array_key_exists( 'array_storage_value_from' , $field_data )
                    &&
                    is_array( $field_data['array_storage_value_from'] )
                ) {

                // -------------------------------------------------------------

                foreach ( $field_data['array_storage_value_from'] as $add_edit => $value_from_details ) {

                    // ---------------------------------------------------------

                    if (    array_key_exists( 'method' , $value_from_details )
                            &&
                            array_key_exists( 'instance' , $value_from_details )
                            &&
                            $value_from_details['method'] === 'post'
                            &&
                            is_string( $value_from_details['instance'] )
                            &&
                            $value_from_details['instance'] !== ''
                            &&
                            array_key_exists( $value_from_details['instance'] , $_POST )
                        ) {

                        if ( \greatKiwi_byFernTec_adSwapper_local_v0x1x211_wordpressMagicQuotes\question_magic_quotes_gpc() ) {

                            $_POST[ $value_from_details['instance'] ] =
                                \stripslashes( $_POST[ $value_from_details['instance'] ] )
                                ;
                        ...
                        $question_field_base64_encoded = TRUE ;
            ...
            if ( $question_field_base64_encoded ) {
                continue ;
            }
...
//pr( $_POST ) ;
    ...
    if ( count( $_POST ) > 0 ) {
        ...
        $old_POST = $_POST ;
            //  $zebra_form_obj->validate() filters/sanitisers $_POST.  Which
            //  then stuffs out the custom/standard submission handling that
            //  follows.
            //
            //  Hence the need to save/restore $_POST.
...
EOT;
                ...
                // -------------------------------------------------------------------------
                // \greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\handle_zebra_form_submission(
                //      $caller_app_slash_plugins_global_namespace      ,
                //      $home_page_title                                ,
                //      $caller_apps_includes_dir                       ,
                //      $all_application_dataset_definitions            ,
            ...
            if ( is_array( $result ) ) {
                ...
                if ( \greatKiwi_byFernTec_adSwapper_local_v0x1x211_stringUtils\starts_with( $error_message , '--ZEBRA--' , $ignore_case_FALSE ) ) {
...
EOT;
                ...
                if ( $return_to === 'manage-dataset' ) {
                    ...
                    // -------------------------------------------------------------------------
                    // \greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\get_show_view_url(
                    //      $caller_apps_includes_dir   ,
                    //      $question_front_end         ,
                    //      $view_slug = NULL
                    //      )
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $selected_datasets_dmdd['zebra_form'] ) ;

    if (    array_key_exists( 'inline_scripts_or_other_html' , $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ] )
            &&
            is_string( $selected_datasets_dmdd['zebra_forms'][  $form_slug_underscored  ]['inline_scripts_or_other_html'] )
            &&
            trim( $selected_datasets_dmdd['zebra_forms'][  $form_slug_underscored  ]['inline_scripts_or_other_html'] ) !== ''
        ) {
        $add_edit_form .= "\n" . trim( $selected_datasets_dmdd['zebra_forms'][  $form_slug_underscored  ]['inline_scripts_or_other_html'] ) ;
    }
...
//      el.parentNode.style.backgroundColor = '#C8F3EB' ;   //  Light Cyan
    ...
    $add_edit_form .= "\n" . <<<EOT
<script type="text/javascript">
    function greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_onfocus(el) {
//      el.parentNode.greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_oldBg = el.parentNode.style.backgroundColor ;
//      el.parentNode.style.backgroundColor = '#D6F6F0' ;   //  Light Cyan
        ...
        ancestor.greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_oldBg = ancestor.style.backgroundColor ;
        ancestor.style.backgroundColor = '#D6F6F0' ;   //  Light Cyan
    }
    function greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_onblur(el) {
        if ( typeof el.parentNode.greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_oldBg !== undefined ) {
//          el.parentNode.style.backgroundColor = el.parentNode.greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_oldBg ;
//          delete el.parentNode.greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_oldBg ;
            ...
            ancestor.style.backgroundColor = ancestor.greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_oldBg ;
...
EOT;
    ...
    $add_edit_form .= <<<EOT
<script type="text/javascript">
    function greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_focus() {
        var error_inputs ;
        {$set_class_for_field_that_caused_error}
    ...
    greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditRecord_zebraForm_focus() ;
</script>
EOT;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $vertical_radio_field_names ) ;
    ...
    if ( count( $vertical_radio_field_names ) > 0 ) {
        ...
        foreach ( $vertical_radio_field_names as $radio_name ) {
            $commands .= <<<EOT
adSwapper_local_v0x1x211_make_zebra_radios_vertical('{$radio_name}') ;\n
EOT;
        }
        ...
        $add_edit_form .= <<<EOT
<script type="text/javascript">
    function adSwapper_local_v0x1x211_make_zebra_radios_vertical( radio_name ) {
return ;
//alert(radio_name);
...
EOT;
    ...
    // =========================================================================
    // Add in any "form fixup javascript"...
    //
    // NOTE!
    // -----
    // The form fixup javascript specified is assumed to be NOT enclosed
    // in SCRIPT tags.
    // =========================================================================
...
//    ) ;
    ...
    if (    array_key_exists(
                'form_fixup_javascript'     ,
                $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]
                )
            &&
            is_string( $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['form_fixup_javascript'] )
            &&
            trim( $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['form_fixup_javascript'] ) !== ''
        ) {

        $add_edit_form .= <<<EOT
<script type="text/javascript">
    {$selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['form_fixup_javascript']}
</script>
EOT;

//echo htmlentities( $add_edit_form ) ;

    }

    // =========================================================================
    // Add in the FIELD GROUPS stuff (if the form has any)...
    // =========================================================================
...
//    ) ;
    ...
    if (    array_key_exists(
        ...
        $form_field_groups = $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['field_groups'] ;
        ...
        // ---------------------------------------------------------------------
                ...
                &&
                is_string( $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['get_field_groups_function_name'] )
                &&
                trim( $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['get_field_groups_function_name'] ) !== ''
                &&
                function_exists( $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['get_field_groups_function_name'] )
        ) {

        // -------------------------------------------------------------------------
        // <my_custom_get_form_field_groups_function>(
        //      $core_plugapp_dirs                      ,
        //      $all_application_dataset_definitions    ,
        //      $dataset_slug                           ,
        //      $question_front_end                     ,
        ...
        $form_field_groups =
            $selected_datasets_dmdd['zebra_forms'][ $form_slug_underscored ]['get_field_groups_function_name'](
                $core_plugapp_dirs                      ,
                $all_application_dataset_definitions    ,
                $dataset_slug                           ,
                $question_front_end                     ,
        ...
        if ( is_string( $form_field_groups ) ) {
    ...
    if (    is_array( $form_field_groups )
            &&
            count( $form_field_groups ) > 0
        ) {

        // ---------------------------------------------------------------------

        require_once( dirname( __FILE__ ) . '/zebra-form-field-group-support.php' ) ;

        // ---------------------------------------------------------------------

        $result = get_zebra_form_field_group_support( $form_field_groups ) ;
    ...
    $zebra_form_master_dir_url =
        \greatKiwi_byFernTec_adSwapper_local_v0x1x211_pathUtils\wp_path2url( $zebra_form_master_dir_path ) ;
...
//  window.parent.document = document ;
    window.parent.window.parent.document.getElementById( 'greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditForm_iframe' ).style.visibility = 'visible' ;
</script>
EOT;
...
EOT;
    ...
    // -------------------------------------------------------------------------
    // \greatKiwi_byFernTec_adSwapper_local_v0x1x211_wordpressPageCache\set_page(
    //      $page_name                      ,
...
//                  ;
    ...
    $page_name =    \greatKiwi_byFernTec_adSwapper_local_v0x1x211_appsAPI\get_plugin_slug_dashed() .
                    '-great-kiwi-standard-dataset-manager-add-edit-form'
                    ;
    ...
    $page_key = \greatKiwi_byFernTec_adSwapper_local_v0x1x211_wordpressPageCache\set_page(
                    $page_name                      ,
                    $question_session_specific      ,
                    $question_remote_ip_specific    ,
                    $question_user_agent_specific   ,
...
EOT;
    ...
    $iframe_src_url =
        \greatKiwi_byFernTec_adSwapper_local_v0x1x211_pathUtils\wp_path2url( $iframe_src_path ) ;
...
*/
    ...
    $iframe_src_url =
        \admin_url( 'admin-ajax.php' ) . <<<EOT
?action=greatKiwi_byFerntec_getCachedPage&page_name={$page_name}&page_key={$page_key}
EOT;
        ;
    ...
    if ( function_exists( '\greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\is_raw_mode_for_forms' ) ) {

        // ---------------------------------------------------------------------

        if ( \greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager\is_raw_mode_for_forms() === TRUE ) {
            $raw_mode_is   = 'ON'  ;
            $turn_raw_mode = 'OFF' ;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $display_options ) ;
    ...
    if ( $question_show_table_header ) {
        ...
        if (    isset( $_GET['return_to'] )
                ...
                trim( $_GET['return_to'] ) !== ''
                &&
                \greatKiwi_byFernTec_adSwapper_local_v0x1x211_stringUtils\ctype_alphanumeric_underscore_dash( $_GET['return_to'] )
                &&
                isset( $_GET['view_slug'] )
                &&
                trim( $_GET['view_slug'] ) !== ''
                &&
                \greatKiwi_byFernTec_adSwapper_local_v0x1x211_stringUtils\ctype_alphanumeric_underscore_dash( $_GET['view_slug'] )
            ) {
    ...
    // =========================================================================
    // Display any dataset-specific add/edit record page header...
    // =========================================================================
    ...
    if (    array_key_exists( 'custom_add_edit_record_page_header_fn' , $zebra_form_def )
            &&
            is_string( $zebra_form_def['custom_add_edit_record_page_header_fn'] )
            &&
            trim( $zebra_form_def['custom_add_edit_record_page_header_fn'] ) !== ''
        ) {

        // ---------------------------------------------------------------------

        if ( ! function_exists( $zebra_form_def['custom_add_edit_record_page_header_fn'] ) ) {

            $msg = <<<EOT
PROBLEM:&nbsp; Bad "custom_add_edit_record_page_header_fn" (function not found)
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
        ...
        $custom_header =
            $zebra_form_def['custom_add_edit_record_page_header_fn'](
                $core_plugapp_dirs                      ,
                $all_application_dataset_definitions    ,
                $selected_datasets_dmdd                 ,
                $dataset_slug                           ,
    ...
    //          ...
    //          'pre_display_function'      =>  '\\' . __NAMESPACE__ . '\\some_function_name'      ,
    //          'pre_display_function_args' =>  <any PHP type>
    //          ...
    //          )
    //
    // -------------------------------------------------------------------------

    if (    array_key_exists( 'pre_display_function' , $selected_datasets_dmdd )
            &&
            is_string( $selected_datasets_dmdd['pre_display_function'] )
            &&
            trim( $selected_datasets_dmdd['pre_display_function'] ) !== ''
        ) {

        // ---------------------------------------------------------------------

        if ( ! function_exists( $selected_datasets_dmdd['pre_display_function'] ) ) {

            $ln = __LINE__ - 2 ;

            $msg = <<<EOT
PROBLEM:&nbsp; Bad "pre_display_function" (function not found)
...
EOT;
        ...
        if ( array_key_exists( 'pre_display_function_args' , $selected_datasets_dmdd ) ) {

            $pre_display_function_args = $selected_datasets_dmdd['pre_display_function_args'] ;
        ...
        // apart from via any Javascript it returns.
        //
        ...
        $pre_display_function_returned_js =
            $selected_datasets_dmdd['pre_display_function'](
                $core_plugapp_dirs                      ,
                $all_application_dataset_definitions    ,
                $display_options                        ,
                $submission_options                     ,
        ...
        if ( is_array( $pre_display_function_returned_js ) ) {

            return standard_dataset_manager_error(
                        $home_page_title                        ,
                        $pre_display_function_returned_js[0]    ,
                        $caller_apps_includes_dir               ,
                        $question_front_end
                        ) ;
    ...
    // =========================================================================
    // Display the add/edit record form (in an IFRAME)...
    // =========================================================================
    ...
    echo <<<EOT
<iframe
    id="greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditForm_iframe"
    src="{$iframe_src_url}"
    width="98%"
...
<br />
<script type="text/javascript">
    function adSwapper_local_v0x1x211_adjust_iframe_height() {
...
//alert( new_iframe_height ) ;
    ...
    setTimeout( "adSwapper_local_v0x1x211_adjust_iframe_height()" , 1000 ) ;
    setTimeout( "adSwapper_local_v0x1x211_adjust_iframe_height()" , 3000 ) ;
    setTimeout( "adSwapper_local_v0x1x211_adjust_iframe_height()" , 9000 ) ;
</script>
{$pre_display_function_returned_js}
EOT;
        //  NOTE!
        //  -----
        //  The id="greatKiwi_byFernTec_adSwapper_local_v0x1x211_standardDatasetManager_addEditForm_iframe" is
