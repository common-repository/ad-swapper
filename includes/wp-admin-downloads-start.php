...
<div style="color:#AA0000">
    ...
    <a href="{$return_screen_url}" style="text-decoration:none"><big><b>OK</b></big></a>
</div>
EOT;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $current_user , '$current_user' ) ;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $meta_keys ) ;
    ...
    if ( \get_user_meta( $current_user->ID , $meta_keys['plugin_camel_name'] , $single ) !== 'adSwapper_local' ) {
        ...
        return <<<EOT
PROBLEM:&nbsp; "update_user_meta()" or "get_user_meta()" failure saving "plugin_camel_name"
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
    ...
    $result = \update_user_meta( $current_user->ID , $meta_keys['plugin_version_alnum'] , 'v0x1x211' ) ;
                    //  Returns row ID on successful update, FALSE on failure.

    // -------------------------------------------------------------------------

    if ( \get_user_meta( $current_user->ID , $meta_keys['plugin_version_alnum'] , $single ) !== 'v0x1x211' ) {

        return <<<EOT
PROBLEM:&nbsp; "update_user_meta()" or "get_user_meta()" failure saving "plugin_version_alnum"
Detected in:&nbsp; \\{$ns}\\{$fn}()
EOT;
...
//\greatKiwi_byFernTec_adSwapper_local_v0x1x211_testDebug\pr( $string_to_download , '$string_to_download' ) ;
