...
// ***************************************************************************
    ...
    namespace greatKiwi_byFernTec_adSwapper_local_v0x1x211_stringUtils ;
...
function contains( $haystack , $needle , $ignore_case = FALSE ) {
    ...
    // contains( $haystack , $needle , $ignore_case = FALSE )
    // - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // RETURNS
    //      TRUE or FALSE
    // -----------------------------------------------------------------------
...
function to_one_line( $instr , $glue = NULL ) {
    if ( $glue === NULL ) {
        $glue = chr(32) ;
    }
    return implode( $glue , to_lines( $instr ) ) ;
        //  NOTE!
        //  =====
        //  The default $glue = chr(32) (= ASCII SPace) is in case a bunch of
        //  lines with NLs stripped has a word at the end of one line - and the
        //  next word at the start of the next line.  If we join the lines with
        //  the empty string, then the words will be joined together.
}
...
function to_one_tight_line( $instr ) {
    return implode( '' , to_lines( $instr ) ) ;
}
...
function to_orange_message( $message , $prefix = '' , $suffix = '' ) {
    ...
    if ( is_string( $message ) ) {
        ...
        if ( $message !== '' ) {
            ...
            return <<<EOT
{$prefix}
<div style="background-color:#FFF0DD; border:1px solid #552F00; color:#552F00; padding:0.3em 1em">
    {$message}
</div>
{$suffix}
EOT;
...
function ctype_dashed_name( $value ) {
...
function hex_clean(
    ...
    for ( $i=0 ; $i<$j ; $i++ ) {
        if ( \ctype_xdigit( $instr[$i] ) ) {
            $clean .= $instr[$i] ;
        } elseif ( $question_display_bad_chars ) {
            echo '<br />---bad char---' , \ord( $instr[$i] ) ;
        }
    }
...
function array_values_to_list(
    ...
    foreach ( $array as $index => $value ) {
        ...
        if ( $index === 0 ) {
            ...
            $this_separator = $last_separator ;

        } else {
            $this_separator = $separator ;

        }

        // ---------------------------------------------------------------------

        $list .= $this_separator . encircle( $value , $quotes ) ;
